...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_limit = torch.tensor(50, device=device)  # adjust force_limit to 50 (orr experiment with different values)
    
    # Introduce force_reward_temp and give graded reward based on force magnitude (scaled by force_limit)
    force_reward_temp = torch.tensor(1.0, device=device)
    force_penalty = torch.clamp(force_magnitude / force_limit, 0, 1)
