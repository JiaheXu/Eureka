...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize large force based on the object angular velocity (rewritten component)
    force_reward_temp = torch.tensor(0.1, device=device)
    dof_force_norm = torch.clamp(torch.norm(dof_force_tensor, dim=1, keepdim=True), min=1e-5)
    scaled_dof_force = torch.div(dof_force_tensor, dof_force_norm)
    force_penalty = torch.mean(torch.square(scaled_dof_force), dim=1) * torch.abs(object_angvel).mean(dim=1)
