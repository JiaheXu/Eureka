...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Compute force magnitude and penalty (updated component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_limit = torch.tensor(100.0, device=device)
    force_penalty_temp = torch.tensor(0.01, device=device)  # experiment with different values
    force_penalty = torch.where(force_magnitude > force_limit, 
                                force_penalty_temp * (force_magnitude - force_limit), 
