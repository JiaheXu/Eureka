...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    orientation_reward_temp = torch.tensor(5.0, device=device)  # increased the temperature parameter
    orientation_reward = torch.exp(-orientation_reward_temp * rot_error_sum)
    
    # Penalize large angular velocities
    angvel_reward_temp = torch.tensor(0.05, device=device)  # changed the temperature parameter
    ...
    force_reward_temp = torch.tensor(0.01, device=device)  # increased the temperature parameter
    normalized_dof_force = torch.div(dof_force_tensor, torch.norm(dof_force_tensor, dim=1, keepdim=True))
    force_penalty = torch.sum(torch.square(normalized_dof_force), dim=1)  # changed to sum instead of mean
