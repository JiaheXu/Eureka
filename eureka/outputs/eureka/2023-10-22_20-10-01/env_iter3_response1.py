...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_reward_temp = torch.tensor(10.0, device=device) # increased temperature parameter
    dof_force_square_mean = torch.square(torch.mean(dof_force_tensor, dim=1))
    dof_force_penalty = torch.sum(torch.square(dof_force_tensor), dim=1) - dof_force_square_mean
