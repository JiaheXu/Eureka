...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize total joint force applied by the hand (modified component)
    force_reward_temp = torch.tensor(0.001, device=device)
    joint_force_penalty = torch.sum(torch.abs(dof_force_tensor), dim=1)
    force_reward = torch.exp(-force_reward_temp * joint_force_penalty)
