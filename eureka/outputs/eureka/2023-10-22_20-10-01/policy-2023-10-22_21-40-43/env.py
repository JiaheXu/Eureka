...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_v3(self.object_rot, self.goal_rot, self.object_angvel, self.dof_force_tensor)
...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    orientation_reward_temp = torch.tensor(0.5, device=device)  # adjust the temperature parameter
    orientation_reward = torch.exp(-orientation_reward_temp * rot_error_sum)
    
    # Penalize large angular velocities
    angvel_reward_temp = torch.tensor(0.05, device=device)  # adjust the temperature parameter
    ...
    force_reward_temp = torch.tensor(0.005, device=device)  # increase the temperature parameter
