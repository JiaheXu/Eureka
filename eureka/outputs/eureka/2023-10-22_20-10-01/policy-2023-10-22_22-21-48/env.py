...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_reward_temp = torch.tensor(0.1, device=device) # temperature parameter
    dof_force_squared = torch.square(dof_force_tensor)
    dof_force_abs_mean = torch.mean(torch.abs(dof_force_tensor), dim=1)
    dof_force_penalty = torch.where(dof_force_abs_mean > 0.01, torch.mean(dof_force_squared, dim=1), torch.tensor(0.0, device=device))
