...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Compute the force magnitude and introduce force constraint modification (updated component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_limit = torch.tensor(100, device=device)  # use soft force_limit of 100 instead of hard limit
    force_temp = torch.tensor(0.01, device=device)  # introduce force_temp variable to control the force scaling
    force_penalty = torch.maximum(force_magnitude - force_limit, torch.tensor(0.0, device=device))
    force_reward = torch.exp(-force_temp * force_penalty)
