...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_limit = torch.tensor(100, device=device)  # introduce force_limit of 100, experiment with different values
    force_temp = torch.tensor(0.005, device=device)  # introduce a temperature parameter for the force component
    force_penalty = torch.exp(torch.clip(force_temp * (force_magnitude - force_limit), min=0)) - 1
