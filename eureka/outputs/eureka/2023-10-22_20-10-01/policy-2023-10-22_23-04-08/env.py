...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor,
    ...
    # Compute the force magnitude and penalize values greater than force_limit (updated component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_limit = torch.tensor(100, device=device)  # keep the force_limit of 100 as previous
    force_penalty = torch.where(force_magnitude > force_limit, force_magnitude - force_limit, torch.tensor(0.0, device=device))
    
    force_reward_temp = torch.tensor(0.01, device=device)  # tuned to place appropriate weight on force_penalty
