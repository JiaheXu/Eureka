...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor,
    ...
    force_reward_temp = torch.tensor(0.04, device=device) # decrease temperature parameter
    dof_force_sum = torch.sum(torch.abs(dof_force_tensor), dim=1)
    normalized_dof_force_sum = dof_force_sum / torch.max(dof_force_sum)
    force_reward = torch.exp(-force_reward_temp * normalized_dof_force_sum)
