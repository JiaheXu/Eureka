...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Incentivize rotation alignment (increase temperature from 1.0 to 2.0)
    ...
    # Compute the force magnitude and apply a continuous force penalty (updated component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_penalty_temp = torch.tensor(0.005, device=device)  # increase force penalty temperature
    force_penalty = force_penalty_temp * force_magnitude
