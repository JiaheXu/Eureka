...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_v3(self.object_rot, self.goal_rot, self.object_angvel)
...
@torch.jit.script
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    orientation_reward_temp = torch.tensor(2.0, device=device)  # increased the temperature parameter
    orientation_reward = torch.exp(-orientation_reward_temp * rot_error_sum)
    
    # Penalize large angular velocities
    angvel_reward_temp = torch.tensor(0.5, device=device)  # increased the temperature parameter
