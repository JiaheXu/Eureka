...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    orientation_reward_temp = torch.tensor(1.5, device=device)
    orientation_reward = torch.exp(-orientation_reward_temp * rot_error_sum)
    
    # Penalize large angular velocities
    angvel_reward_temp = torch.tensor(0.05, device=device)
    angvel_penalty = torch.sum(torch.square(object_angvel), dim=1)
    angvel_reward = torch.exp(-angvel_reward_temp * angvel_penalty)

    # Re-scale and penalize large forces applied by the hand
    force_reward_temp = torch.tensor(0.0005, device=device)
    force_penalty = torch.sum(torch.square(dof_force_tensor), dim=1) / 1000.0  # Re-scale the force_penalty
