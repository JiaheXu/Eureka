...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    orientation_reward_temp = torch.tensor(2.0, device=device)  # Modified temperature
    orientation_reward = torch.exp(-orientation_reward_temp * rot_error_sum)
    
    # Penalize large angular velocities
    angvel_reward_temp = torch.tensor(0.05, device=device)  # Modified temperature
    ...
    force_reward_temp = torch.tensor(0.01, device=device)  # Increased penalty sensitivity
