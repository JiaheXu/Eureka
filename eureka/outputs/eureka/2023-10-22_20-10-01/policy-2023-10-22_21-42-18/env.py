...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Incentivize rotation alignment (increased temperature and scaled reward)
    orientation_reward_temp = torch.tensor(2.0, device=device)
    orientation_reward = torch.exp(-orientation_reward_temp * rot_error_sum)
    orientation_reward_scale = torch.tensor(2.0, device=device)
    orientation_reward = orientation_reward_scale * orientation_reward
    ...
    # Penalize large force applied by the hand (increased temperature)
