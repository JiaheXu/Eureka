...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_ratio = torch.abs(dof_force_tensor / torch.max(torch.abs(dof_force_tensor), dim=1, keepdim=True)[0])  # scale dof_force_tensor
    force_penalty = torch.mean(torch.square(force_ratio), dim=1)
