...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    angvel_reward_temp = torch.tensor(0.15, device=device)  # increased the temperature parameter
    angvel_penalty = torch.sum(torch.square(object_angvel), dim=1)
    angvel_reward = torch.exp(-angvel_reward_temp * angvel_penalty)

    # Penalize large force magnitudes applied by the hand (updated component)
    force_reward_temp = torch.tensor(0.001, device=device)
    force_magnitudes = torch.norm(dof_force_tensor, dim=1)
    high_force_penalty = torch.square(torch.clamp(force_magnitudes - 10.0, min=0.0))
    force_reward = torch.exp(-force_reward_temp * high_force_penalty)
