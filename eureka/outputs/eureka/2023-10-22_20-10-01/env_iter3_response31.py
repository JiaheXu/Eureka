...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_reward_temp = torch.tensor(1.0, device=device)  # updated temperature parameter
    dof_force_squared_sum = torch.sum(torch.square(dof_force_tensor), dim=1)  # sum of squared forces
    dof_force_penalty = dof_force_squared_sum / torch.max(dof_force_squared_sum)  # normalized force penalty
