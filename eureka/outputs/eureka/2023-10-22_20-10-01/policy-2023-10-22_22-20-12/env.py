...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize large total force applied by the hand (updated component)
    force_reward_temp = torch.tensor(0.5, device=device) # increased temperature parameter
    total_force = torch.sum(torch.abs(dof_force_tensor), dim=1)
    max_total_force = torch.max(total_force) + 1e-8  # to avoid division by zero
    normalized_total_force = total_force / max_total_force
    force_reward = torch.exp(-force_reward_temp * normalized_total_force)

    # Combine reward components with different weights
    reward = (0.6 * orientation_reward + # increased the weight for orientation_reward
              0.3 * angvel_reward +
              0.1 * force_reward) # reduced the weight for force_reward
