...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Compute the force magnitude and improve force reward component (updated component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    desired_force_magnitude = torch.tensor(100, device=device)  # experiment with different values
    force_penalty_scale = torch.tensor(0.01, device=device)  # scaling factor for force penalty, experiment with different values
    force_penalty = force_penalty_scale * torch.square(force_magnitude - desired_force_magnitude)
    force_reward = torch.exp(-force_penalty)
