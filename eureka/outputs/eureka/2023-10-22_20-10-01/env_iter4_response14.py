...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_v5(self.object_rot, self.goal_rot, self.object_angvel, self.episode_length)
...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
                      object_angvel: torch.Tensor, episode_length: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Incentivize rotation alignment (updated orientation_reward_temp)
    ...
    # New time_reward component
    max_episode_length = 1000
    time_reward_temp = torch.tensor(0.005, device=device)
    time_reward = torch.exp(-time_reward_temp * (max_episode_length - episode_length))

    # Combine reward components
    reward = orientation_reward * angvel_reward * time_reward
