...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_penalty_temp = torch.tensor(0.001, device=device)
    dof_force_l1_penalty = torch.mean(torch.abs(dof_force_tensor), dim=1)
    force_penalty = torch.exp(-force_penalty_temp * dof_force_l1_penalty)

    # Combine reward components
    reward = orientation_reward * angvel_reward * force_penalty
