...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_threshold = torch.tensor(0.5, device=device)
    dof_force_penalty = torch.mean(torch.square(dof_force_tensor), dim=1)
    force_reward = torch.where(dof_force_penalty < force_threshold, 
                               torch.ones_like(dof_force_penalty), 
                               torch.zeros_like(dof_force_penalty))
