...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    angvel_reward_temp = torch.tensor(0.1, device=device)  # keep the temperature parameter
    ...
    force_reward_temp = torch.tensor(0.5, device=device)  # increased the temperature parameter
    force_square_sum = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_penalty = torch.div(force_square_sum, torch.norm(dof_force_tensor, dim=1))
