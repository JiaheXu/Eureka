...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_v4(self.object_rot, self.goal_rot, self.object_angvel, self.dof_force_tensor)
...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize large force applied by the hand (revised component)
    force_reward_temp = torch.tensor(1.0, device=device) 
    dof_force_squared = torch.square(dof_force_tensor)
    dof_force_penalty = torch.mean(dof_force_squared, dim=1) / torch.max(dof_force_squared)
    force_reward = torch.exp(-force_reward_temp * dof_force_penalty)

    # Encourage smaller net force (new component)
    net_force_reward_temp = torch.tensor(0.1, device=device)
    net_force_penalty = torch.sum(torch.abs(dof_force_tensor), dim=1)
    net_force_penalty_normalized = net_force_penalty / torch.max(net_force_penalty)
    net_force_reward = torch.exp(-net_force_reward_temp * net_force_penalty_normalized)

    # Combine reward components
    reward = orientation_reward * angvel_reward * force_reward * net_force_reward

    reward_components = {"orientation_reward": orientation_reward,
                         "angvel_reward": angvel_reward,
                         "force_reward": force_reward,
                         "net_force_reward": net_force_reward}
