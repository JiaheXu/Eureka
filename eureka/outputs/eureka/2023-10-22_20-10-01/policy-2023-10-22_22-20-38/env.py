...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    angvel_reward_temp = torch.tensor(0.5, device=device)  # updated temperature parameter
    angvel_penalty = torch.sum(torch.square(object_angvel), dim=1)
    angvel_reward = torch.exp(-angvel_reward_temp * angvel_penalty)

    # Penalize large force applied by the hand in contact with the object (new component, replaces force_reward)
    force_magnitude_reward_temp = torch.tensor(0.01, device=device)  # added temperature parameter
    force_magnitude_penalty = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_magnitude_reward = torch.exp(-force_magnitude_reward_temp * force_magnitude_penalty)

    # Combine reward components
    reward = orientation_reward * angvel_reward * force_magnitude_reward

    reward_components = {"orientation_reward": orientation_reward,
                         "angvel_reward": angvel_reward,
                         "force_magnitude_reward": force_magnitude_reward}
