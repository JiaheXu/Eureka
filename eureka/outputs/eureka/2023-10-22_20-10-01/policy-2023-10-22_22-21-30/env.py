...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    orientation_reward_temp = torch.tensor(0.5, device=device) # decreased temperature parameter
    orientation_reward = torch.exp(-orientation_reward_temp * rot_error_sum)
    
    # Penalize large angular velocities
    angvel_reward_temp = torch.tensor(0.05, device=device) # decreased temperature parameter
    angvel_penalty = torch.sum(torch.square(object_angvel), dim=1)
    angvel_reward = torch.exp(-angvel_reward_temp * angvel_penalty)

    # Penalize large force applied by the hand (new component)
    force_reward_temp = torch.tensor(0.01, device=device)  # decreased temperature parameter
    dof_force_norm = torch.norm(dof_force_tensor, dim=1)
    dof_force_penalty = dof_force_norm - torch.min(dof_force_norm)
