...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize high variance in the force applied by the hand (new component)
    force_reward_temp = torch.tensor(5.0, device=device)
    force_mean = torch.mean(dof_force_tensor, dim=1)
    force_var = torch.var(dof_force_tensor, dim=1)
    force_reward = torch.exp(-force_reward_temp * force_var)
