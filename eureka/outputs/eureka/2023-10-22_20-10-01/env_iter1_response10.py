...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    angvel_reward_temp = torch.tensor(0.05, device=device)  # Increased temperature parameter
    ...
    # Encourage spinning
    spin_reward_temp = torch.tensor(1.0, device=device)
    spin_reward = torch.exp(spin_reward_temp * torch.abs(torch.sum(object_angvel, dim=1)))

    # Combine reward components
    reward = orientation_reward * angvel_reward * force_reward * spin_reward
