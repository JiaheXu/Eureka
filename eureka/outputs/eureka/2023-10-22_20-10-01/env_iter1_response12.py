...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_rot, self.goal_rot, self.object_angvel, self.dof_force_tensor)
...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    angvel_reward_temp = torch.tensor(0.02, device=device)  # increased from 0.01
    angvel_penalty = torch.sum(torch.square(object_angvel), dim=1)
    angvel_reward = torch.exp(-angvel_reward_temp * angvel_penalty)

    # Penalize large force applied by the hand (updated force_reward)
    force_reward_temp = torch.tensor(0.0005, device=device)  # decreased from 0.001
    force_penalty = torch.sum(torch.square(dof_force_tensor), dim=1) - 0.01  # subtract a threshold to make it more sensitive
    force_penalty = torch.clamp(force_penalty, min=0.0)  # clip penalties below the threshold
