...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Compute the force magnitude (updated component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_reward_temp = torch.tensor(0.0001, device=device)  # introduce force_reward_temp, experiment with different values
