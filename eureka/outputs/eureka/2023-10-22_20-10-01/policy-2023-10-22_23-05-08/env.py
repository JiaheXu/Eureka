...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize large force magnitudes (updated component)
    force_reward_temp = torch.tensor(0.01, device=device)  # introduce force_reward_temp
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_penalty = torch.sum(torch.abs(force_magnitude))
