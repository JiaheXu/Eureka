...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Compute the force magnitude and subtract the target_force from it (updated component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    target_force = torch.tensor(50, device=device)  # introduce target_force of 50, experiment with different values
    force_error = torch.abs(force_magnitude - target_force)
    
    # Penalize deviations from the target force (updated component)
    force_reward_temp = torch.tensor(0.01, device=device)
    force_reward = torch.exp(-force_reward_temp * force_error)
