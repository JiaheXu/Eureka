...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_v5(self.object_rot, self.goal_rot, self.object_angvel, self.dof_force_tensor)
...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize high object_rot_dist (new component)
    object_rot_dist_temp = torch.tensor(0.5, device=device)
    object_rot_dist = torch.sum(torch.square(goal_rot - object_rot), dim=1)
    object_rot_dist_penalty = torch.exp(-object_rot_dist_temp * object_rot_dist)

    # Compute the force magnitude and introduce force_limit
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_limit = torch.tensor(100, device=device)
    force_penalty = torch.relu(force_magnitude - force_limit) / force_limit
    
    # Update force reward to penalize excessive force (updated component)
    force_reward_temp = torch.tensor(0.1, device=device)
    force_reward = torch.exp(-force_reward_temp * force_penalty)

    # Combine reward components
    reward = orientation_reward * angvel_reward * object_rot_dist_penalty * force_reward

    reward_components = {"orientation_reward": orientation_reward,
                         "angvel_reward": angvel_reward,
                         "object_rot_dist_penalty": object_rot_dist_penalty,
