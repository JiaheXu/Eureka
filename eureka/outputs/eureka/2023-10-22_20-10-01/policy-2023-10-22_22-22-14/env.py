...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    dof_force_scaled = dof_force_tensor / torch.max(torch.abs(dof_force_tensor))
    force_penalty = torch.mean(torch.square(dof_force_scaled), dim=1)
    force_reward = torch.exp(-force_reward_temp * force_penalty)

    # Combine reward components
    combined_reward = orientation_reward * angvel_reward * force_reward

    # Normalize the reward to a fixed range by applying a scaling factor
    reward_scale = torch.tensor(10.0, device=device)
    reward = reward_scale * combined_reward
