...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    angvel_reward_temp = torch.tensor(0.1, device=device)  # increased the temperature parameter
    ...
    force_reward_temp = torch.tensor(0.1, device=device)  # increased the temperature parameter
    force_penalty = torch.mean(torch.abs(dof_force_tensor), dim=1)
