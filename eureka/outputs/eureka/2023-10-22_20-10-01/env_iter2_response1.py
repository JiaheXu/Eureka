...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Incentivize rotation alignment (increased influence)
    ...
    # Penalize large force applied by the hand (re-writing the force_reward component)
    force_reward_temp = torch.tensor(0.01, device=device)  # increased temperature parameter
    force_penalty = torch.mean(torch.pow(dof_force_tensor, 2), dim=1)
