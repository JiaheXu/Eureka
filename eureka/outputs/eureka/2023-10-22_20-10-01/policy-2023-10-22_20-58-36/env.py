...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_v2(self.object_rot, self.goal_rot, self.object_angvel)
...
def compute_reward_v2(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    orientation_reward_temp = torch.tensor(5.0, device=device)  # Increased temperature
    orientation_reward = torch.exp(-orientation_reward_temp * rot_error_sum)
    
    # Penalize large angular velocities
    angvel_reward_temp = torch.tensor(0.1, device=device)  # Increased temperature
