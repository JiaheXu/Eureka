...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Compute the force magnitude and introduce adaptive force_limit (updated component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    adaptive_scale = torch.tensor(50.0, device=device)
    force_limit = torch.relu(adaptive_scale * (1 - orientation_reward)) + torch.tensor(10.0, device=device)
