...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Compute force efficiency reward (new component)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_efficiency_reward_temp = torch.tensor(0.001, device=device)  # introduce a new temperature for force efficiency reward
    force_efficiency_reward = torch.exp(-force_efficiency_reward_temp * force_magnitude)
    
    # Combine reward components
    reward = orientation_reward * angvel_reward * force_efficiency_reward
