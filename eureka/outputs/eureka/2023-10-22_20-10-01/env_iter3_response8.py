...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_reward_temp = torch.tensor(0.2, device=device) # increased temperature parameter
    dof_force_penalty = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_measure = dof_force_penalty
    sharpen_augment = torch.tensor(10.0, device=device) # new parameter to sharpen distinction between high and low forces
    force_measures_augmented = torch.pow(force_measure + 1.0, sharpen_augment)
    normalized_force_measures = force_measures_augmented / torch.max(force_measures_augmented)
    force_reward = torch.exp(-force_reward_temp * normalized_force_measures)
