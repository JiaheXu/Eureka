...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_v3(self.object_rot, self.goal_rot, self.object_angvel, self.dof_force_tensor, self.step_count, self.max_steps)
...
def compute_reward_v3(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
                      object_angvel: torch.Tensor, dof_force_tensor: torch.Tensor,
                      step_count: torch.Tensor, max_steps: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    force_reward_temp = torch.tensor(0.1, device=device)  # increased temperature parameter
    force_penalty_weight = torch.tensor(100.0, device=device)  # increased penalty weight
    force_penalty = torch.mean(torch.square(dof_force_tensor), dim=1)
    force_reward = torch.exp(-force_reward_temp * force_penalty_weight * force_penalty)

    # Penalize longer task completion time
    time_penalty_temp = torch.tensor(0.01, device=device)
    time_penalty = step_count / max_steps
    time_reward = torch.exp(-time_penalty_temp * time_penalty)

    # Combine reward components
    reward = orientation_reward * angvel_reward * force_reward * time_reward
