...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize large angular velocities using an exponential penalty
    ...
    # Encourage force minimization with an exponential penalty (updated component)
    force_temp = torch.tensor(0.01, device=device)  # introduce a separate temperature parameter for the force component
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_reward = torch.exp(-force_temp * force_magnitude)
