...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Penalize large force applied by the hand (new approach for force_reward component)
    force_reward_temp = torch.tensor(0.001, device=device)
    dof_force_norm = torch.norm(dof_force_tensor, dim=1)
    dof_force_penalty = torch.mean(torch.square(dof_force_norm - dof_force_norm.mean()), dim=0)
