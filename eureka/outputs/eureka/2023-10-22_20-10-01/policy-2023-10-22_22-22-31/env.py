...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_reward_temp = torch.tensor(0.01, device=device) # increased temperature parameter
    force_magnitudes = torch.sqrt(torch.sum(torch.square(dof_force_tensor), dim=1))
    force_threshold = torch.tensor(10.0, device=device) # soft threshold for force magnitudes
    force_penalty = torch.maximum(torch.zeros_like(force_magnitudes, device=device), force_magnitudes - force_threshold) # apply the soft threshold
