...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_improved(self.object_rot, self.goal_rot, self.object_angvel, self.dof_force_tensor)
...
def compute_reward_improved(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    angvel_reward_temp = torch.tensor(0.1, device=device)  # increase weight for the angular velocity component
    angvel_penalty = torch.sum(torch.square(object_angvel), dim=1)
    angvel_reward = torch.exp(-angvel_reward_temp * angvel_penalty)

    # Penalize large force applied by the hand (change scale and temperature for force_reward)
    force_reward_temp = torch.tensor(0.01, device=device)  # decrease weight for the force component
