...
def compute_reward_v5(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    # Update the force reward (incremental penalty based on deviation from desired limit)
    force_magnitude = torch.sum(torch.square(dof_force_tensor), dim=1)
    force_limit = torch.tensor(100, device=device)  # desired limit of force magnitude
    force_penalty = torch.abs(force_magnitude - force_limit)
    force_reward_temp = torch.tensor(0.01, device=device)  # update force_reward_temp value
