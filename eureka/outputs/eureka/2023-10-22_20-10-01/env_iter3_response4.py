...
def compute_reward_v4(object_rot: torch.Tensor, goal_rot: torch.Tensor, 
    ...
    force_reward_temp = torch.tensor(0.01, device=device)  # Adjusted temperature parameter
    force_threshold = torch.tensor(0.5, device=device)  # New force threshold value
    dof_force_penalty = torch.mean(torch.square(dof_force_tensor), dim=1)
    force_excess = torch.max(dof_force_penalty - force_threshold, torch.zeros_like(dof_force_penalty))
    force_reward = torch.exp(-force_reward_temp * force_excess)
