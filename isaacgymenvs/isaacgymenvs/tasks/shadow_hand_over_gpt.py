...
class ShadowHandOverGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_gpt(self.object_pos, self.object_rot, self.object_linvel, self.goal_pos, self.goal_rot, self.fingertip_pos, self.fingertip_another_pos) 
        self.extras['gpt_reward'] = self.rew_buf.mean()
        for rew_state in self.rew_dict: self.extras[rew_state] = self.rew_dict[rew_state].mean()
        # def compute_reward_gpt(object_pos: torch.Tensor, object_rot: torch.Tensor, object_linvel: torch.Tensor,
