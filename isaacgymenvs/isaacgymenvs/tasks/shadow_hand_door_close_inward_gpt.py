...
class ShadowHandDoorCloseInwardGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_gpt(self.goal_pos, self.door_left_handle_pos, self.door_right_handle_pos, self.left_hand_pos, self.right_hand_pos) 
        self.extras['gpt_reward'] = self.rew_buf.mean()
        for rew_state in self.rew_dict: self.extras[rew_state] = self.rew_dict[rew_state].mean()
        #def compute_reward_gpt(goal_pos: torch.Tensor, door_left_handle_pos: torch.Tensor, door_right_handle_pos: torch.Tensor, left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor)
