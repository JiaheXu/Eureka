...
class ShadowHandDoorOpenOutwardGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_gpt(self.left_hand_pos, self.door_left_handle_pos, self.door_left_handle_rot) 
        self.extras['gpt_reward'] = self.rew_buf.mean()
        for rew_state in self.rew_dict: self.extras[rew_state] = self.rew_dict[rew_state].mean()
        #def compute_reward_gpt(left_hand_pos: Tensor, door_left_handle_pos: Tensor, door_left_handle_rot: Tensor) -> Tuple[Tensor, Dict[str, Tensor]]:
