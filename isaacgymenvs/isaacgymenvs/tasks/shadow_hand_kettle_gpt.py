...
class ShadowHandKettleGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_gpt(self.kettle_handle_pos, self.kettle_spout_pos, self.bucket_handle_pos, self.left_hand_pos, self.right_hand_pos) 
        self.extras['gpt_reward'] = self.rew_buf.mean()
        for rew_state in self.rew_dict: self.extras[rew_state] = self.rew_dict[rew_state].mean()
        # def compute_reward_gpt(kettle_handle_pos: torch.Tensor, kettle_spout_pos: torch.Tensor, bucket_handle_pos: torch.Tensor,
